...
2XX：成功（请求正常处理完毕）  
200：OK（请求成功），浏览器协商缓存中，如果缓存资源有更新，则服务端会返回对应更新的资源，且状态码是200。
202：接收到了请求，但是尚未进行处理
...
304：（常考：协商缓存）缓存资源没有更新时服务器端返回304(Not Modified)。
...
#### `http1.0`：
* 每次发包都需要重新握手和挥手。一次tcp连接只能发送接受一个请求和请求响应。（ 一次请求 - 响应，建立一个连接，用完关闭；每一个请求都要建立一个连接；）
**缺点：** 连接无法复用，导致每次请求都需要经历三次握手；
#### `http1.1`：
* http请求头增加了`keep-alive`：保持连接(持续连接)。只要我们建立连接，那么在当前这个连接内只需要一次握手和一次撒手。即支持持久连接。(即在`一个TCP连接`上可以传送`多个HTTP请求和响应`，减少了建立和关闭连接的消耗和延迟)
* `http1.1`请求数据包的发送是串行。
...
* 关于缓存：在`http1.0`中主要使用header里的`If-Modified-Since,Expires`来做为缓存判断的标准，HTTP1.1则引入了更多的缓存控制策略例如`Entity tag，If-Unmodified-Since, If-Match, If-None-Match`等更多可供选择的缓存头来控制缓存策略。
**缺点**：
* 若干个请求排队串行化单线程处理，后面的请求等待前面请求的返回才能获得执行机会，一旦有某请求超时等，后续请求只能被阻塞，毫无办法，也就是人们常说的线头阻塞；（只能一次一次来，没有什么优先策略，只能排队，非常公平，长请求（大请求）和短请求（小请求）都一样公平排队，效率低。）
* 页面整体出来，但是白页到整个页面出来的时间可能很长。
#### `http2.0`：多个请求可以同时在一个连接（即tcp连接）上并行执行，当某个请求任务耗时严重时，不会影响到其它连接的正常执行；同等条件下，小请求有抢资源的机会，不会一直排队被阻塞。页面一点一点加载出来。
* `http2.0`多路复用（MultiPlexing），即连接共享，即每一个request都是用作连接共享机制的。一个request对应一个id，这样一个连接上可以有多个request，每个连接的request可以随机的混杂在一起，接收方可以根据request的 id将request再归属到各自不同的服务端请求里面。因此小请求有抢占资源的机会，不会一直排队被阻塞。可以看到页面或者图片是一点点加载出来的。
* `http2.0`可以同时多个请求并行发送或响应。
* `http2.0`支持传输二进制内容，不仅限于文本内容。
* `http2.0`支持服务器推送，比如`index.html`文件中引入了一个`main.js`文件，当客户端请求`index.html`资源时，服务器会把`index.html`文件依赖的`main.js`文件一并推送给客户端。这样就省去了客户端重复请求的过程，也就没有建立连接的过程，这样可以很大程度提升资源传输效率。
#### `http3.0`
* tcp（重试是客户端重试） ——> udp(解决不可靠是通过不断重试)	
...
http/https是位于网络结构层中的应用层协议，是一种超文本传输协议。`http2.0`也支持传输二进制文件。
tcp/udp是位于网络结构层次中传输层的传输协议，传输的是二进制内容。

### 浏览器对同一个域名（Host）建立 TCP 连接到数量有没有限制？
Chrome一般限制6个tcp连接，不同浏览器不同（因为浏览器内核实现不同）。
有的浏览器限制是4个tcp连接。
超过浏览器连接数量限制后就会被阻塞。
